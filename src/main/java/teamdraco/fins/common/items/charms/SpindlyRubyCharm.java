...
public class SpindlyRubyCharm extends ArmorItem implements ISpindlyCharmItem {
    public static final IArmorMaterial MATERIAL = new FinsArmorMaterial(FinsAndTails.MOD_ID + ":spindly_ruby_charm", 1, new int[]{1, 2, 3, 1}, 8, SoundEvents.ITEM_ARMOR_EQUIP_CHAIN, 0.0F, null);
    ...
    public void onArmorTick(ItemStack stack, World world, PlayerEntity player) {
        if (player.isAlive() && player.getHealth() <= 4.0F && !player.getCooldownTracker().hasCooldown(this)) {
            player.addPotionEffect(new EffectInstance(Effects.FIRE_RESISTANCE, 140, 0, false, false, true));
